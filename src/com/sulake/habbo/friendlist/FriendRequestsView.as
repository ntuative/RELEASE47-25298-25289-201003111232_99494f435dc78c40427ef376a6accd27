package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1417:String = "no_reqs_info";
       
      
      private var var_1565:IContainerButtonWindow;
      
      private var _friendList:HabboFriendList;
      
      private var var_1564:IContainerButtonWindow;
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_284 && _loc4_.state != FriendRequest.const_301)
            {
               _loc3_.addAcceptedRequest(_loc4_.requestId);
               _loc4_.state = FriendRequest.const_284;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
                  _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_54,_loc4_.requestId));
               }
            }
         }
      ...
      {
         var_1565 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         var_1564 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         var_1564.procedure = onDismissAllButtonClick;
         var_1565.procedure = onAcceptAllButtonClick;
         refreshButtons();
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_284 && _loc4_.state != FriendRequest.const_301)
            {
               _loc4_.state = FriendRequest.const_301;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
      ...
      private function refreshButtons() : void
      {
         ...
         setEnabled(var_1565,_loc1_);
         setEnabled(var_1564,_loc1_);
      }
      ...
      public function acceptRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_284;
         ...
         if(_friendList && false)
         {
            _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_54,param1));
         }
      }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_301;
         var _loc3_:DeclineBuddyMessageComposer = new DeclineBuddyMessageComposer();
