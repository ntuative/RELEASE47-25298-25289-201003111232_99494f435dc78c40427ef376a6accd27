package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public static const const_315:int = 100;
      ...
      {
         return Math.ceil(this.var_224.length / const_315);
      }
      ...
      {
         if(this.var_828 >= this.getPageCount())
         {
            this.var_828 = Math.max(0,this.getPageCount() - 1);
         }
      }
      ...
      {
         var _loc4_:* = null;
         if(var_224.indexOf(param1) >= 0)
         {
            return;
         }
         var _loc2_:String = param1.name.toLowerCase();
         var _loc3_:int = 0;
         while(_loc3_ < var_224.length)
         {
            _loc4_ = var_224[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               var_224.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         checkPageIndex();
         return Math.min((this.var_828 + 1) * const_315,this.var_224.length);
      }
